...
if [ $# -eq 0 ]; then
    ...
    HYBRID_INFO=$(cat /etc/alpine-raspbian-hybrid 2>/dev/null || echo "Standard system")
    
    echo "📊 Pi-Star A/B System Status:"
    echo "  • Active partition: $CURRENT_PART"
    echo "  • Version: $CURRENT_VERSION"
    echo "  • System type: $HYBRID_INFO"
    echo "  • Root device: $(mount | grep ' / ' | cut -d' ' -f1)"
    
    # Show kernel information
    if [ -d "/boot/kernelA" ] || [ -d "/boot/kernelB" ]; then
        echo "  • Kernel management: A/B kernel directories"
        if [ -d "/boot/kernel$CURRENT_PART" ]; then
            KERNEL_COUNT=$(ls /boot/kernel$CURRENT_PART/kernel*.img 2>/dev/null | wc -l)
            echo "  • Available kernels: $KERNEL_COUNT in partition $CURRENT_PART"
        fi
    else
        echo "  • Kernel management: Shared kernels"
    fi
